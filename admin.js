...
// Ouverture / Fermeture classique du groupe
...
// Appui Long pour afficher le tableau des cotes
...
function toggleCotesPanelExplicit(groupId) {
  ...
  if (el) {
    ...
    el.classList.toggle('show-cotes');
  }
}
...
// Calcul des gains du groupe + Mise à jour du badge d'en-tête du groupe
function applyCotesFilterAndCalculate(groupId) {
  ...
  // Mise à jour du Total Gain dans le tableau et sur l'en-tête du groupe
...
function renderPoste(tickets, prefix) {
  ...
  keys.forEach(key => {
    ...
    if (headerEl) setupLongPressHandlers(headerEl, groupId);
...
// Sauvegarde dans la base et fermeture du tableau de cotes
  ...
  try {
    ...
    if (error) {
      ...
      if (btn) {
        ...
        setTimeout(() => {
          btn.innerText = "✅ OK / Enregistrer & Quitter";
          btn.style.backgroundColor = "#047857";
        }, 2000);
      }

      // Quitter le tableau après la sauvegarde tout en gardant le gain affiché sur le groupe
      const groupCard = document.getElementById(groupId);
      if (groupCard) {
