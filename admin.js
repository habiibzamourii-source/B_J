...
// Clic sur l'icône 📈 : Ouvre/ferme le tableau des cotes
...
function renderPoste(tickets, prefix) {
  ...
  filteredTickets.forEach(ticket => {
    ...
    // Normalisation robuste pour gérer les accents, espaces et apostrophes (ex: Les Sables d'Olonne)
  ...
  keys.forEach((key, index) => {
    ...
    const cotesToggleBtnHTML = isCotesView
      ? `<button type="button" class="btn-toggle-cotes" onclick="toggleCotesPanel(event, '${groupId}')" title="Ouvrir/Fermer le Tableau des Cotes" style="background: rgba(255,255,255,0.1); border: 1px solid var(--border); padding: 4px 8px; border-radius: 6px; cursor: pointer; font-size: 1rem; margin-right: 8px;">📈</button>`
      : '';
