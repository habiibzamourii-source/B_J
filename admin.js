...
// Restaure le dernier onglet sélectionné depuis le localStorage (par défaut 'all')
let currentTab = localStorage.getItem('active_tab_preference') || 'all';

// Suivi global des gains par poste
let accumulatedGainsP1 = 0;
let accumulatedGainsP2 = 0;

function switchTab(tab) {
  currentTab = tab;
  localStorage.setItem('active_tab_preference', tab); // Sauvegarde persistante
...
function toggleGroup(event, groupId) {
  if (event && event.target.closest('.btn-cotes-toggle')) return;
...
// 2. Ouverture / Fermeture explicite via le bouton Cote
...
function updateConsolidatedGainsAndProfit() {
  ...
  const cardProfit = document.getElementById('kpi-profit-card');
  ...
  if (cardProfit) {
    if (beneficeDuJour < 0) {
      cardProfit.classList.add('loss');
    } else {
      cardProfit.classList.remove('loss');
    }
  }
}
...
function renderTicketItem(t, index, prefix) {
  ...
  const badgePosteHTML = originPoste === 'P1'
  ...
  return `
    <div class="ticket-wrapper ${isCanc ? 'is-cancelled' : ''}" 
      ...
      <div class="ticket-row" onclick="toggleTicketDetail(event, '${ticketUniqueId}')">
        <div class="code-pill">🎫 ${t.code || 'SANS-CODE'}</div>
        <div class="ticket-location-tag">
          ${badgePosteHTML}
          <span class="tag-pays">${pays}</span>
          <span class="tag-course">${course}</span>
        </div>
        <div class="t-date">${t.ticket_date || 'N/A'}</div>
        <div class="t-bets">${renderBetsSummary(t)}</div>
        <div class="t-amount">
      ...
      <div class="ticket-detail-pane">
        <div class="receipt-box ${isCanc ? 'is-cancelled' : ''}">
          <div class="receipt-header">
            <span class="receipt-code"><code>CODE: ${t.code || 'N/A'}</code></span>
          ...
          <div class="bets-list">${blocksHTML || '<div class="empty-state">Aucun pari enregistré</div>'}</div>
          <div class="receipt-footer">
            <span class="status-tag ${isCanc ? 'cancelled' : 'printed'}">${isCanc ? 'ANNULÉ' : 'VALIDE'}</span>
            <span class="receipt-total">${amt.toLocaleString('fr-FR')} DT</span>
          </div>
        </div>
      </div>
    </div>
...
function renderPoste(tickets, prefix) {
  ...
  keys.forEach(key => {
    ...
    const btnCotesHTML = isCotesView
      ? `<button class="btn-cotes-toggle" onclick="toggleCotesPanel(event, '${groupId}')">📈 Cotes</button>`
      : '';
    ...
    card.innerHTML = `
      <div class="group-header" onclick="toggleGroup(event, '${groupId}')">
...
// 3. Sauvegarde dans cotes_gains et fermeture automatique du tableau des cotes
